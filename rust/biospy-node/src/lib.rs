// napi-rs Node-API bridge. Exposes biospy-core's pure functions to Node
...
//     so end users `npm install biospy` and get the right .node file with
...
// binaries (biospy / biospy-mcp). Node consumers only need this if they want
// to call biospy from JS without spawning a subprocess.

use napi_derive::napi;

#[napi]
pub fn version() -> String {
    biospy_core::version().to_string()
}
