...
fn suggest_recovery_strategy(checks: &[HealthCheck]) -> Vec<RecoveryStep> {
        ...
        steps.push(RecoveryStep {
            ...
            action:
                "Image appears blank or all-zero — re-read the chip with better SOIC clip contact"
                    .to_string(),
            command: "biospy read new_dump.bin --safe".to_string(),
            risk: Risk::Low,
        });
        return steps;
    }
    ...
    if fails.iter().any(|c| c.name == "UEFI Firmware Volumes") {
        steps.push(RecoveryStep {
            ...
            action: "No valid firmware found — obtain correct BIOS from manufacturer and reflash"
                .to_string(),
            command: "biospy write correct_bios.bin".to_string(),
            risk: Risk::Medium,
        });
        order += 1;
    }
        ...
        steps.push(RecoveryStep {
            ...
            action:
                "Region boundary error — image may be truncated. Re-read chip and verify full size"
                    .to_string(),
            command: "biospy read full_dump.bin && biospy analyze full_dump.bin".to_string(),
            risk: Risk::Low,
        });
        order += 1;
    }
        ...
        .any(|c| c.name == "Intel ME" && c.status == CheckStatus::Fail)
    {
        steps.push(RecoveryStep {
            ...
            command: "biospy region-extract donor.bin me --output donor_me.bin && biospy region-replace corrupt.bin me donor_me.bin --output fixed.bin".to_string(),
            risk: Risk::Medium,
        });
        order += 1;
    }
            ...
            RecoveryStep {
                ...
                action:
                    "Image has warnings but no critical failures — may be usable. Flash and test"
                        .to_string(),
                command: "biospy write dump.bin && biospy verify dump.bin".to_string(),
                risk: Risk::Low,
            }
        } else {
            RecoveryStep {
                ...
                command: "biospy verify dump.bin".to_string(),
                risk: Risk::Low,
            }
        });
    }
