// Mock backend — in-memory flash emulation. Used for tests and `BIOSPY_FORCE_MOCK=1`.
    ...
    fn write_chip(&mut self, input_path: &Path, opts: WriteOpts) -> Result<WriteResult> {
        ...
        let backup_path = if opts.skip_backup {
            ...
            let ts = SystemTime::now()
                ...
                .map(|d| d.as_millis())
                .unwrap_or(0);
            let path = std::env::temp_dir().join(format!("biospy-backup-mock-{}.bin", ts));
...
mod tests {
    ...
    fn tmp_file(name: &str) -> PathBuf {
        std::env::temp_dir().join(format!("biospy-mock-test-{}-{}", std::process::id(), name))
    }
